...
%
Results clearly show that controlling the diversity of the variable space is a way to improve further the results 
...
%
Such analyses are designed to test the scalability in the variable space, the performance with different stopping criteria, 
...
were configured with $24$ parameters, where $20$ of them correspond to distance parameters and $4$ to position parameters.
%
...
%
Note that the experiment devoted to analyze the scalability in the variables, consider different amount of variables.
...
%
Note that in the supplementary material, results are also compared in terms of the IGD+ metric and conclusions are quite similar.
%
...
\begin{table}[t]
\centering
\caption{ Parameterization applied in each MOEA}
...
%
Moreover, it is more impressive that the mean \HV{} ratio attained by \VSDMOEA{} when considering all the problems simultaneously is much larger
...
%
The calculated data confirms that although \VSDMOEA{} loses in some cases, the overall numbers of wins and losses
favors \VSDMOEA{}.
%
More importantly, the total deterioration is quite lower in the case of \VSDMOEA{} confirming that when \VSDMOEA{} loses the deteriation is not 
that large.
...
In order to study the scalability of \VSDMOEA{} in terms of the number of variables, all the already described algorithms were tested with
...
%
Specifically, the number of distance parameters were set to $42$, $84$, and $210$ in the cases of using $50$, $100$ and $250$ variables, respectively.
%
...
%
In the two-objective case, the deterioration is similar in every algorithm, so the superiority of \VSDMOEA{} is clear regardless of the amount of 
variables.
%
...
\caption{Mean of the \HV{} ratio of 35 runs considering the two-objective problems considering several numbers of variables}\label{fig:variable-decision-scalability-2obj}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[scale=0.85]{Images/Graphic-Scalability-3obj_tikz-figure0.eps}
\caption{Mean of the \HV{} ratio of 35 runs considering the three-objective problems considering several numbers of variables} \label{fig:variable-decision-scalability-3obj}
\end{figure}

In order to better understand this behavior, we selected the WFG1 to WFG7 problems.
%
WFG problems divide the variables in two kinds of parameters (this framework uses the term parameter instead of variable): the distance parameters and the position parameters.
%
...
\end{equation}
%
This is really important because it has been shown that for these kinds of cases, state-of-the-art
...
%
In order to not saturate this Figures, only the information of \VSDMOEA{} and \RMOEA{} with 50, 100 and 250 variables is shown.
%
The behavior of \NSGAII{} and \MOEAD{} --- which are not included --- is similar to the one of \RMOEA{} in terms of the ADI evolution. 
%
...
%
Moreover, increasing the number of variables, provokes an important increase in the attained ADI, meaning that in this case,
...
\end{tabular}
\caption{Performance of \MOEAS{} for the problems with two objectives considering three ranges of stopping criterion: short-term (first row), middle-term (second row) and long-term (third row).}\label{fig:Performance_time_2obj}
\end{figure}
...
One of the disadvantages of including a strategy to control the diversity is that this is usually done at the cost of
