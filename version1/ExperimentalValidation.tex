...
%
At the beginning, several theoretical especifications are explained, as is widely used in MOEAs comparison outline.
%
Therafter, with the motivation of describe in detail the benefits offered by our proposal (especifically for long-term executions) the following characteristics are analyzed.
%
The scalability in the decision variable space is taken into account.
%
This analyzes is narrowed through some specific and well known problems.
%
In the same line, with the intention to have a better understanding of the initial factor distance parameter ($D_I$) several settings of this parameter with all the test-problems are reported .
%
Finally, since that the mechanism imposed in our proposal, which avoid the premature convergency, highly depends on the elapsed time, we report the benefits of our proposal considering both short and long term executions.
%
Additionally, the latter experiments shows that the \VSDMOEA{} has a decent performance in short-term executions.
%
...
% 
The UF benchmark is composed of ten test instances, which is categorized in two groups that are based in the number of objetive functions to solve, thus the first seven consists of two objectives and the reaming problems of three objectives, where the number of decision variables taken into account in each one is $n=30$.
%
...
\multirow{3}{*}{\textbf{MOEA/D}} & Neighbour size = 10, max. update by sub-problem (nr) = 2, \\
 & probability local selection ($\delta$) = 0.9, tour selection = 10, \\
 & period utility updating = 30 generations \\ \hline
