...
This section is devoted to fully describe our novel proposal called \textit{Variable Space Diversity based MOEA} (\VSDMOEA{}) and its source
code is freely available~\footnote{The code in C++ can be downloaded in the next link \url{https://github.com/joelchaconcastillo/VSD-MOEA.git}}.
%
...
%
In such a case, the individual with a larger contribution to variable space diversity is selected.
...
%
This front (refered in Algorithm~\ref{alg:Density_Estimator}  as $F$) contains at least one individual belonging to $R_t$ and it might also contain some elements 
of $P_{t+1}$.
%
...
%
Mainly, the sets $FP$ and $FR$ are employed refering to the survivors and candidates individuals that belong to the current front $F$ respectively (lines \ref{alg:FR} and \ref{alg:FS}).
%
...
%
Denoting as $FP$ the selected members of the input front that are already in $P_{t+1}$ (line \ref{alg:FS}), the contribution of each member which belongs to the input front and that has not been selected ($R_t$) is calculated as $\displaystyle{\min_{s \in FP}\ ID(I, s)}$.
%
...
\STATE Input: $P_{t+1}$ (Survivors), $R_t$ (Candidates), $F$ (Current front)
	...
	\STATE $FP = P_{t+1} \cap F$ \label{alg:FS}
