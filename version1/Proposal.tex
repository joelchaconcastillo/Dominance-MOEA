...
%				\ENDFOR
        \WHILE{ $|P_{t+1}|$ $\leq$ N } \label{alg:6}
        	...
        	\ENDIF
					...
					\STATE Use the novel density estimator (Algorithm~\ref{alg:Replacement_Phase}) to select a new survivor 
...
that contributes significantly in terms of objective-space quality and diversity and belongs to $R_t$.

Algorithm~\ref{alg:density-estimator} describes the selection of the next survivor.
%
...
given in Eq.~\ref{eqn:extremes} is calcualated (line XXX).
%
...
and the process finalizes (line YYY).
%
...
to objective-space diversity is calculated by taking into account the current survivors of the front (line XXX).
%
...
is used.
%
The $ID$ of an individual $A$ with respect to an individual $B$ is calculated by taking into account only the functions
where $A$ is better.
%
...
%
Denoting as $FS$ the members of the input front that are already in $P_{t+1}$, the contribution to diversity
of each member of the input front that belongs to $R_t$ is calculated as $\displaystyle{\min_{s \in S}\ ID(I, s)}$.
%
Then, the individual with a higher contribution is selected as the next survivor, with ties broken randomly (line XXX).
