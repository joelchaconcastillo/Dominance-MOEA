...
plot for [i=0:STATS_blocks-1] "BOXPLOT/VSDMOEA_2" using (6*i):2 index i lt 1 lc rgb 'white'title (i==0 ? 'VSD-MOEA' : ''),\
     for [i=0:STATS_blocks-1] "BOXPLOT/CPDEA_2" using (6*i+1):2 index i lt 1 lc rgb 'blue' title (i==0 ? 'CPDEA' : ''),\
     for [i=0:STATS_blocks-1] "BOXPLOT/MOEAD_2" using (6*i+2):2 index i lt 1 lc rgb 'red'title (i==0 ? 'MOEA/D' : ''),\
     for [i=0:STATS_blocks-1] "BOXPLOT/R2EMOA_2" using (6*i+3):2 index i lt 1 lc rgb 'green' title (i==0 ? 'R2-EMOA' : ''),\
     for [i=0:STATS_blocks-1] "BOXPLOT/CPDEA_2" using (6*i+2):(-1):xticlabel(1) index i w l notitle
