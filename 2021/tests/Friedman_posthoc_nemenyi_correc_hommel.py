...
def process_instance(list_files):
  ...
  if p < alpha: 
    ptable = sp.posthoc_nemenyi(data)
...
def Friedman_Posthoc_nemeyi_score():
...
def Friedman_Posthoc_nemeyi_info():
  print("Friedman test with posthoc nemeyi and holm correction")
  list_files = open(sys.argv[1], "r")
  for inst in list_files.read().split('\n'):
    if len(inst)==0:
        continue
...
Friedman_Posthoc_nemeyi_score()
#Friedman_Posthoc_nemeyi_info()
